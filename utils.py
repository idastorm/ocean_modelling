...
def simulate_vessel_trajectory(vessel, timesteps, dt, day, currents, winds):
    ...
    for t in np.arange(timesteps):
        ...
        v_x_current = current_in_x((t, longitude, latitude))
        v_y_current = current_in_y((t, longitude, latitude))
        ...
        # r = np.random.uniform(0.1, 1, 1000)
        # theta = np.linspace(0, 2*np.pi, 1000)
        # x, y = longitude - r * np.cos(theta), latitude - r * np.sin(theta)
        # longitudes = x
        # latitudes  = y
                
        # If we have not reached land, get the wind velocity
        
        # v_x_current = np.nanmean(current_in_x((day+t, longitudes, latitudes)))
        # v_y_current = np.nanmean(current_in_y((day+t, longitudes, latitudes)))

        # v_x_wind = np.nanmean(wind_in_x((day+t, longitudes, latitudes)))
        # v_y_wind = np.nanmean(wind_in_y((day+t, longitudes, latitudes)))


        v_x_wind = wind_in_x((t, longitude, latitude))
        v_y_wind = wind_in_y((t, longitude, latitude))                
    ...
    return {"trajectories": vessel, "drifts": drifts}
