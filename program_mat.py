...
if __name__ == '__main__':
    ...
    mask = np.flip(loadmat('Mask')['Mask'][:,:,0], axis=0)
    ...
    all_dates_current_x = all_dates_current_x[:, lat_mask][:,:, lon_mask]
    all_dates_current_y = all_dates_current_y[:, lat_mask][:,:, lon_mask]

    all_dates_wind_x = all_dates_wind_x[:, lat_mask][:,:, lon_mask]
    all_dates_wind_y = all_dates_wind_y[:, lat_mask][:,:, lon_mask]
    ...
    for launch_day in tqdm(days):
        ...
        current_in_x = RegularGridInterpolator((lat, lon), all_dates_current_x[launch_day, :, :], bounds_error=False, fill_value=np.nan)
        current_in_y = RegularGridInterpolator((lat, lon), all_dates_current_y[launch_day, :, :], bounds_error=False, fill_value=np.nan)
                
        wind_in_x = RegularGridInterpolator((lat, lon), all_dates_wind_x[launch_day, :, :], bounds_error=False, fill_value=np.nan)
        wind_in_y = RegularGridInterpolator((lat, lon), all_dates_wind_y[launch_day, :, :], bounds_error=False, fill_value=np.nan)
